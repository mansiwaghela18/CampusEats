...
public class Floor1MenuActivity extends AppCompatActivity {
    ...
    // Replace your existing methods with these fixed versions:
...
// Remove these duplicate methods - they're causing the issue:
// - private void FilterMenuItems()
// - private void SetupMenu()
// - private void addNoItemsFoundMessage()
    ...
    private void addNoItemsFoundMessage() {
        ...
        noItemsText.setTextColor(Color.parseColor("#666666"));
        noItemsText.setGravity(android.view.Gravity.CENTER);
        noItemsText.setPadding(dpToPx(16), dpToPx(32), dpToPx(16), dpToPx(32));

        menuContainer.addView(noItemsText);
    }
    ...
    private void addMenuItemCard(MenuDataManager.MenuItem item) {
        ...
        // Set initial button state based on current stock from data manager
        int currentStock = menuDataManager.getItemStock(item.id);
    ...
    private void addToCart(MenuDataManager.MenuItem item) {
        ...
        if (currentStock > 0) {
            // Decrease stock in data manager using the enhanced method
            boolean stockDecreased = menuDataManager.decreaseStock(item.id, 1);
            ...
            if (stockDecreased) {
                ...
                Toast.makeText(this, "Failed to add item - insufficient stock!", Toast.LENGTH_SHORT).show();
                // Refresh menu to show correct stock status
                refreshMenuData();
            }
        } else {
